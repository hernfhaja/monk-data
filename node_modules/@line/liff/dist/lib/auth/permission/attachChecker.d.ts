import { Permission } from '../../store/isPermissionAvailable';
declare type defaultFnType = (...args: unknown[]) => Promise<unknown>;
/**
 * Create wrapped API function with permission checking and it will grant the permission automaticallly before calling the API.
 * API Reference
 * @export
 * @returns {Promise<unknown>}
 */
export default function attachChecker<FnType extends defaultFnType>(fn: FnType, permission: Permission): FnType;
export {};
